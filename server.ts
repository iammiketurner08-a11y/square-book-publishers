...
async function startServer() {
  ...
  const getResend = () => {
    ...
    if (!apiKey) {
      console.error("[Server] CRITICAL: RESEND_API_KEY is missing from environment variables.");
      throw new Error("Email service is not configured. Please add RESEND_API_KEY to the Settings menu in AI Studio.");
    }
    if (!resend) {
